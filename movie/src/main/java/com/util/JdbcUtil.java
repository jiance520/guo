...
public class JdbcUtil {//工具类，针对不同的数据库，使用同样的jdbc方法。
	...
	private static String url = "jdbc:postgresql://localhost:5432/geodb";
...
//	使用构造方法初始化工具类,必须是非静态属性
     ...
     * @return 受影响的行数
     */
	public static int executeUpdate(Connection conn,String sql,Object... params){//String...比Object[]更方便，直接传任意个参数
     ...
     * @return 结果集
     */
	public static HashMap<String,Object> queryOne(Connection conn,String sql,Object... params){
     ...
     * @return 结果集
     */
	public static ResultSet executeQueryRS(Connection conn,String sql,Object... params){
		try {
...
//	exectueQuery("select * from emp where id=?","12");
	public static List<HashMap> exectueQuery(Connection conn,String sql,Object... params){
...
//					用getColumnLabel查出的是咱们在后面重新定义的字段名，如果没有定义就是默认字段名，并且是大写字符串！
     ...
     */
	public static HashMap<String,Object> queryProcedure(Connection conn,String sql,int outParamPos,int sqlType,Object... params){
		HashMap<String,Object> map = null;
		ResultSetMetaData rsmd = null;
		int columnCount = 0;
		try {
...
//	函数的返回值只有一个，不像过程可以返回游标，所以函数返回的只能是一个Object.
	public static Object queryFunction(Connection conn,String sql,int outParamPos,int sqlType,Object... params){
		Object obj = null;
		int columnCount = 0;
		try {
			logger.debug("开始调用函数");
...
//	查询表允许为空的字段，请传入表名如PRODUCT，
	public static List<HashMap> nullList(Connection conn,String tablename){
		String sql = "select column_name from user_tab_columns where table_name = '"+tablename+"' and nullable = 'Y'";
		List<HashMap> maplist = exectueQuery(conn,sql);
